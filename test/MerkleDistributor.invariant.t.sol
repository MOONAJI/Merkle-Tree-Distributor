...
contract MerkleDistributorInvariantTest is Test {
    ...
    function setUp() public {
        ...
        distributor.createDistribution(
            bytes32(uint256(1)), 10_000 ether, address(token), block.timestamp, block.timestamp + 365 days
        );
    ...
    function invariant_ClaimedNeverExceedsTotalYield() public view {
        for (uint256 i = 0; i < distributor.distributionCount(); i++) {
            (, uint256 totalYield,,, uint256 claimedAmount,,) = distributor.distributions(i);

            assertLe(claimedAmount, totalYield, "Claimed exceeds total yield");
        }
    }
    ...
    function invariant_TokenBalanceConservation() public view {
        ...
        assertEq(contractBalance + totalClaimedSum, totalYieldsSum, "Token balance not conserved");
    }
        ...
        for (uint256 i = 0; i < claimers.length; i++) {
            ...
            for (uint256 distId = 0; distId < distributor.distributionCount(); distId++) {
                if (handler.hasUserClaimed(distId, claimer)) {
                    assertTrue(distributor.hasClaimed(distId, claimer), "Claimed but hasClaimed is false");
                }
            }
        }
    }
    ...
    function invariant_DistributionCountMonotonic() public view {
        ...
        assertGe(currentCount, handlerCount, "Distribution count decreased");
    }
    ...
    function invariant_ActiveDistributionsValidTimeWindows() public view {
        for (uint256 i = 0; i < distributor.distributionCount(); i++) {
            (,,, bool active,, uint256 startTime, uint256 endTime) = distributor.distributions(i);

            if (active) {
                assertLt(startTime, endTime, "Invalid time window for active distribution");
            }
        }
    }
    ...
    function invariant_MerkleRootNeverZero() public view {
        for (uint256 i = 0; i < distributor.distributionCount(); i++) {
            ...
            assertNotEq(merkleRoot, bytes32(0), "Merkle root is zero");
        }
    }
    ...
    function invariant_OwnerConsistency() public view {
        assertEq(distributor.owner(), owner, "Owner changed unexpectedly");
    }
    ...
    function invariant_TotalClaimedMonotonic() public view {
        for (uint256 i = 0; i < distributor.distributionCount(); i++) {
            (,,,, uint256 claimedAmount,,) = distributor.distributions(i);

            uint256 handlerClaimed = handler.ghost_claimedPerDistribution(i);

            assertGe(claimedAmount, handlerClaimed, "Claimed amount decreased");
        }
    }
    ...
    function invariant_AtLeastOneClaimAttempted() public view {
        assertGt(handler.ghost_totalClaimAttempts(), 0, "No claim attempts occurred");
    }
}
...
contract Handler is Test {
    ...
    constructor(MerkleDistributor _distributor, MockERC20 _token, address _owner) {
     ...
     */
    function claim(uint256 actorSeed, uint256 distributionId, uint256 amount, uint256 proofSeed)
        public
        useActor(actorSeed)
        countCall
    {
     ...
     */
    function claimMultiple(uint256 actorSeed, uint256 numClaims) public useActor(actorSeed) countCall {
        address actor = actors[bound(actorSeed, 0, actors.length - 1)];
     ...
     */
    function createDistribution(uint256 amount, uint256 duration) public {
     ...
     */
    function setDistributionActive(uint256 distributionId, bool active) public {
    ...
    function hasUserClaimed(uint256 distributionId, address user) public view returns (bool) {
...
contract MerkleDistributorAdvancedInvariantTest is Test {
    ...
    function setUp() public {
        ...
        for (uint256 i = 0; i < 5; i++) {
            distributor.createDistribution(
                bytes32(uint256(i + 1)), 100_000 ether, address(token), block.timestamp, block.timestamp + 365 days
            );
        }
    ...
    function invariant_TotalDistributedEqualsSum() public view {
        ...
        assertEq(contractBalance + totalClaimed, totalInitial, "Total distributed mismatch");
    }
    ...
    function invariant_OncePerDistribution() public view {
        ...
        for (uint256 i = 0; i < actors.length; i++) {
            ...
            assertLe(claimCount, distributor.distributionCount(), "User claimed more than once per distribution");
        }
    }
}
